...
lab_dim = 1
# tf Graph Input
input_features = tf.placeholder(tf.float32, [None, input_dim]) # 占位符，行数不确定（batch），列数为input_dim = 2（年纪，肿瘤大小）
input_lables = tf.placeholder(tf.float32, [None, lab_dim])
# Set model weights
W = tf.Variable(tf.random_normal([input_dim,lab_dim]), name="weight") # W:2×1矩阵
